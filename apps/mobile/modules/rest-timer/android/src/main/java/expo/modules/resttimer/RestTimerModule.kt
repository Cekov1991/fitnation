...
class RestTimerModule : Module() {
  ...
  override fun definition() = ModuleDefinition {
    ...
    Function("start") { endAtMillis: Long, label: String ->
      arm(RestTimerService.armIntent(context, endAtMillis, label))
    }

    // Same as start; the label rides along so a service the OS killed and
    // restarted still names the exercise.
    Function("update") { endAtMillis: Long, label: String ->
      arm(RestTimerService.armIntent(context, endAtMillis, label))
    }
  ...
  // caller schedules a local notification instead.
